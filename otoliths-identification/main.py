...
class MakeSquare:
    """Picklable transform for Windows multiprocessing"""
    def __call__(self, img):
        return ut.make_square(img)
...
def main(args):
    ...
    input_trans = transforms.Compose([
        ...
        transforms.RandomRotation(
            30, interpolation=transforms.InterpolationMode.BILINEAR, expand=True),
    ...
    gr_0_lab = ['Ammodytes tobianus', 'Hyperoplus lanceolatus', 'Ammodytes marinus']  # sandeels
    gr_1_lab = ['Clupea harengus', 'Sprattus sprattus', 'Alosa fallax']  # clupeids  
    gr_lab = ['Sandeels', 'Clupeids']  # group names
    ...
    temp = np.isin(all_idx, gr_1_idx).astype(int)
    ...
    # Choose between standard and attention model
    if args.use_attention:
        log.write("=> Using Attention ResNet18 with CBAM\\n")
        from model.CNNs import FineTuneModel_Hierarchical_Attention
        model = FineTuneModel_Hierarchical_Attention(
            model, args.arch, intput_args, len(gr_0_idx), len(gr_1_idx))  
    else:
        log.write("=> Using Standard Hierarchical Model\\n")
        model = FineTuneModel_Hierarchical(
            model, args.arch, intput_args, len(gr_0_idx), len(gr_1_idx))               

    # optimizer - Using Lion optimizer (more memory efficient than Adam)
    optimizer = Lion(
...
if __name__ == '__main__':
    ...
    prs.add_argument('--use_attention', dest='use_attention', default=False,
                action='store_true', help='use attention ResNet18 with CBAM')
