...
class LabelSmoothingCrossEntropy(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        if self.weight is not None:
            ...
            loss = -(true_dist * log_preds).sum(dim=-1) * weight_mask
            return loss.mean()
        else:
...
def make_prediction_per_batch(data_loader, model):
    ...
    return np.array(preds).astype(int), np.array(gprobs), np.array(probs), np.array(pred_gps)
...
def predict(model, outputs, mask=None):
    ...
    prob_sublevel = np.zeros((mask.shape[0], 3), float) # probabilities at second level
    pred_sublevel = np.zeros(mask.shape, int) # predictions at second level
...
def train(train_loader, valid_loader, model, optimizer, args, log=None):
    ...
    # Calculate class weights for balanced learning (helps Alosa fallax)
    # Count samples per class in training set
    class_counts = np.zeros(len(model.args['all_idx']))
    for _, y in train_loader:
        y = y.numpy()
        for label in y:
            idx = model.args['all_idx'].index(label)
            class_counts[idx] += 1
    ...
    log.write(f"Class weights: {class_weights}\\n")
    
    # Create weighted loss for level 1 species classification
    # Group 0 (Sandeels) weights
    weights_gr0 = []
    for idx in model.args['gr_0_idx']:
        pos = model.args['all_idx'].index(idx)
        weights_gr0.append(class_weights[pos].item())
    weights_gr0 = torch.FloatTensor(weights_gr0)
    
    # Group 1 (Clupeids) weights 
    weights_gr1 = []
    for idx in model.args['gr_1_idx']:
        pos = model.args['all_idx'].index(idx)
        weights_gr1.append(class_weights[pos].item())
    weights_gr1 = torch.FloatTensor(weights_gr1)
    
    if conf.GPU_AVAIL:
        weights_gr0 = weights_gr0.cuda()
        weights_gr1 = weights_gr1.cuda()
    
    # Weighted CrossEntropy for both groups
    """CETLoss_gr0 = nn.CrossEntropyLoss(weight=weights_gr0)
    CETLoss_gr1 = nn.CrossEntropyLoss(weight=weights_gr1)"""
    # Label Smoothing CrossEntropy for both groups (smoothing=0.1)
    CETLoss_gr0 = LabelSmoothingCrossEntropy(weight=weights_gr0, smoothing=0.1)
    CETLoss_gr1 = LabelSmoothingCrossEntropy(weight=weights_gr1, smoothing=0.1)

    
    if conf.GPU_AVAIL:
        BCELoss = BCELoss.cuda()
        CETLoss_gr0 = CETLoss_gr0.cuda()
        CETLoss_gr1 = CETLoss_gr1.cuda()
    ...
    for epoch in range(0, args.epochs):
        ...
        # training the model
        loss, acc_level_0, acc_level_1 = run_epoch(train_loader, model, BCELoss, CETLoss_gr0, CETLoss_gr1, optimizer, epoch, args.epochs, log)
        all_train_losses.append(loss)
        all_train_acc_level_0.append(acc_level_0)
        all_train_acc_level_1.append(acc_level_1)

        # validate the valid data
        loss, acc_level_0, acc_level_1,true_labels, pred_labels = evaluate(model, BCELoss, CETLoss_gr0, CETLoss_gr1, valid_loader)
...
def run_epoch(train_loader, model, BCELoss, CETLoss_gr0, CETLoss_gr1, optimizer, epoch, num_epochs, log=None):
    ...
    for batch_idx, (inputs, y) in enumerate(train_loader):
        ...
        # loss at the second level (with class weights)
        if not input_var_0 is None:
            input_var_0 = model.level_1_0(input_var_0)
            loss += CETLoss_gr0(input_var_0, target_var_0) * (target_var_0.data.size(0) / batch_size)

        if not input_var_1 is None:
            input_var_1 = model.level_1_1(input_var_1)
            loss += CETLoss_gr1(input_var_1, target_var_1) * (target_var_1.data.size(0) / batch_size)
...
def evaluate(model, BCELoss, CETLoss_gr0, CETLoss_gr1, data_loader):
...
def categorical_to_binary_tensor(model, labels):
    ...
    mask = np.isin(labels, model.args['gr_1_idx']).astype(int)
