...
# skimage renamed rgb2grey -> rgb2gray in newer versions.
# Provide a robust import / fallback so the code works with both old and new skimage.
try:
    from skimage.color import rgb2gray
except Exception:
    try:
        # older versions / alternate spelling
        from skimage.color import rgb2grey as rgb2gray
    except Exception:
        # ultimate fallback: simple numpy conversion (luminosity)
        import numpy as _np
        def rgb2gray(img):
            """Fallback rgb->gray conversion: handles 2D or 3D arrays."""
            img = _np.asarray(img)
            if img.ndim == 2:
                return img
            if img.ndim == 3 and img.shape[-1] >= 3:
                # use standard luminosity weights (R,G,B)
                return _np.dot(img[..., :3], [0.2989, 0.5870, 0.1140])
            # if unexpected shape, return as-is
            return img
...
# morphology / segmentation imports (robust across skimage versions)
from skimage.morphology import binary_opening, reconstruction
# watershed moved in recent skimage versions to skimage.segmentation
try:
    from skimage.segmentation import watershed
except Exception:
    try:
        # older skimage placed watershed in morphology
        from skimage.morphology import watershed
    except Exception:
        # clear fallback so failure is explicit when function is used
        def watershed(*args, **kwargs):
            raise ImportError("skimage watershed not available. Please install/update scikit-image.")
